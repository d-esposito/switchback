 ...
 */
export function Character({ colors, hatStyle, anim, speedRef }: CharacterProps) {
  ...
  useFrame((_, dt) => {
    ...
    if (anim === "climb") {
      ...
      body.current.position.y = 0.86 + Math.abs(Math.sin(p)) * 0.04;
    } else {
      ...
      body.current.position.y = 0.86 + Math.sin(p * 0.6) * 0.012;
    }
  });
  ...
  return (
    <group>
      ...
      <group ref={legL} position={[-0.11, 0.62, 0]}>
        <mesh position={[0, -0.31, 0]}>
          <boxGeometry args={[0.15, 0.62, 0.17]} />
          <meshStandardMaterial color={colors.pants} flatShading />
        </mesh>
      </group>
      <group ref={legR} position={[0.11, 0.62, 0]}>
        <mesh position={[0, -0.31, 0]}>
          <boxGeometry args={[0.15, 0.62, 0.17]} />
          <meshStandardMaterial color={colors.pants} flatShading />
        </mesh>
