...
export function DayNight() {
  ...
  useFrame(() => {
    ...
    skyScratch.lerp(SKY_DAWN, glow * 0.6);
    (scene.background as THREE.Color).copy(skyScratch);
    if (scene.fog) scene.fog.color.copy(skyScratch);
    ...
    sun.current.intensity = THREE.MathUtils.smoothstep(elev, -0.1, 0.3) * 1.5 + 0.03;
    ...
    moon.current.intensity = (1 - day) * 0.26;
    moon.current.color.copy(MOON);

    hemi.current.intensity = 0.17 + day * 0.6;

    stars.current.visible = elev < 0.08;
  });
