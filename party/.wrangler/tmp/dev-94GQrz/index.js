...
// src/index.ts
var RTC_PATH = /^\/rtc\/(sessions\/new|sessions\/[a-zA-Z0-9]+\/(tracks\/new|tracks\/close|renegotiate))$/;
...
function allowedOrigin(request) {
  ...
  if (origin === "http://localhost:5173" || origin === "http://127.0.0.1:5173" || origin === "https://switchback-game.vercel.app" || origin === "https://hiking-game.vercel.app" || origin.startsWith("https://") && origin.endsWith("-d-espositos-projects.vercel.app")) {
    return origin;
  }
  return null;
}
__name(allowedOrigin, "allowedOrigin");
async function proxyRtc(request, env2) {
  const origin = allowedOrigin(request);
  if (!origin) return new Response("forbidden origin", { status: 403 });
  if (request.method === "OPTIONS") {
    return new Response(null, { status: 204, headers: corsHeaders(origin) });
  }
  if (!env2.CALLS_APP_ID || !env2.CALLS_APP_SECRET) {
  ...
  const upstream = await fetch(
    `https://rtc.live.cloudflare.com/v1/apps/${env2.CALLS_APP_ID}/${m[1]}`,
    {
      method: request.method,
      headers: {
        Authorization: `Bearer ${env2.CALLS_APP_SECRET}`,
...
__name(proxyRtc, "proxyRtc");
var Mountain = class extends Server {
  static {
...
var src_default = {
  async fetch(request, env2) {
    if (new URL(request.url).pathname.startsWith("/rtc/")) {
      return proxyRtc(request, env2);
    }
    return await routePartykitRequest(request, env2) ?? new Response("switchback-party: not a party route", { status: 404 });
  }
};
...
// .wrangler/tmp/bundle-QIMtcv/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-QIMtcv/middleware-loader.entry.ts
